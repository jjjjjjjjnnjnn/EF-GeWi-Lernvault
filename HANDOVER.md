...
- ✅ CC-Switch 风格 LLM 路由、端点自定义与原生协议适配（`[App] 28e4d7b`+本交接）：彻底解决商汤（SenseNova）等代理报错（原生 Anthropic Messages 协议 `/v1/messages` + `x-api-key` + `anthropic-version: 2023-06-01` + 动态 `system` 字段与流式 `content_block_delta` 解析）；开发网关代理 `vite.config.ts` 完整透传所有凭据头与解除 CORS；对齐 CC-Switch 核心配置项（请求地址栏支持「完整 URL」直接请求开关 +「端口 (Port)」自定义绑定 + 上游格式自由切换 + 认证字段下拉选择 `ANTHROPIC_AUTH_TOKEN` / `Bearer` / `x-api-key` / 自定义 Header）；40 单测套件 234 测试 100% 全绿 / build 3.03s 零错误 / vault-check(38/203/135) PASS（Journal 2026-09-23-ccswitch-llm-routing）。
