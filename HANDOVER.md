...
## 当前状态（2026-09-24·顶栏语言热切换移除与全学科教学法体系落地）
- ✅ 顶栏语言热切换移除与全学科教学法体系落地（`[App]`+`[Meta]`+本交接）：①**顶栏语言热切换胶囊移除**：按用户指令移除右上角 DE/中文 胶囊，空间全面释放；语言设置收敛至系统设置（`Settings.tsx`）及全局快捷键（`L`）②**全学科教学法引擎上线 (`fachDidaktik.ts`)**：Gymnasium EF 10 科按四大领域（GeWi / Sprachen / MINT / Mündlich）划分，配备各科标准导向价值裁决、P.E.E. 论据链、现象模型化与四步解题、听觉与动作力学口试备考 ③**新增教具组件**：MINT 规范求解器 `FormulaScaffold.tsx`（已知-公式-代入-解释四步解题，含高考考点预设）+ 口试试场模拟器 `OralExamTimer.tsx`（15 分钟备考+5 分钟独立陈述双模全真计时）④**全局学科过滤联动**：顶栏学科选择器 (`selectedFach`) 联动 `Flashcards`（水平学科卡片计数标签栏）、`Tutor`（自适应展示各科教具按钮与教学法说明徽章）、`Quiz`（学科考点自适应优先）、`KlausurSim`；**55 套件 354 测试 100% 全绿** / `npm run build` 4.59s 零错误 / `vault-check PASS(81/481/200)`（Journal 2026-09-24-didaktik-fach-expansion）。
